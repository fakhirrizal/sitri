...
class MY_Controller extends CI_Controller {
  ...
  function apiCreate($token){  
    ...
    $vars['token'] = '';
      ...
      $url = base_url('index.php?/Jwt/validate/').$token;
      $token = file_get_contents($url);
      $status = json_decode($token, true);
      //echo $d['token_status'];   
      if($status['token_status']=='expired'){
        $generate = base_url('index.php?/Jwt/generate/');
        $new_token = file_get_contents($generate);
